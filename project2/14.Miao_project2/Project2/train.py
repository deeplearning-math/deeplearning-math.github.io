...
from cifar10_data import CIFAR10Random

import cmd_args
import model_mlp, model_wideresnet, model_CNN
...
def get_data_loaders(args, shuffle_train=True):
  if args.data == 'cifar10':
    ...
    train_loader = torch.utils.data.DataLoader(
        CIFAR10Random(root='./data', train=True, download=True,
    ...
    val_loader = torch.utils.data.DataLoader(
        CIFAR10Random(root='./data', train=False,
...
#  lr = args.learning_rate * (0.1 ** (epoch // 150)) * (0.1 ** (epoch // 225))
  lr = args.learning_rate * (0.9 ** (epoch // 10))

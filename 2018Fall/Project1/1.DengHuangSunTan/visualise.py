...
def make_plot(dfs, labels, methods, n_plots=6, s=64, title=None):
    ...
    if n_plots == 1:
        ...
        for (x, ax, method) in zip(dfs, np.reshape(axes,(-1)), methods):
            x_min, x_max = np.min(x, axis=0), np.max(x, axis=0)
            x = (x - x_min) / (x_max - x_min)
            for I,J,K,L in [(r_flags, 'Raphael', 'blue', 'x'),
                          (nr_flags, 'Non-Raphael', 'orange', '+')]:
                xs = x[I, 0]
                ys = x[I, 1]
                ax.scatter(xs, ys, s=s, c=K, marker=L, label=J)
            ax.set_xlabel('')
            ax.set_ylabel('')
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_title(method.upper(), fontsize=42)
            #ax.set_facecolor('grey')
    if title is not None:
        plt.suptitle(title, fontsize=54)
    hdls, lbls = ax.get_legend_handles_labels()
    fig.legend(hdls, lbls, loc='lower center', fontsize=32, markerscale=4)
...
if __name__ == '__main__':
    ...
    features = np.loadtxt('scat_feats.txt')
    labels = features[:,-1]
    for i in methods:
        df = visualize(data=features[:,:-1], method=i, n_jobs=N_jobs)
        np.savetxt('vis_scat/{}.txt'.format(i), df)
    dfs = []
    for i in methods:
        dfs.append(np.loadtxt('vis_scat/{}.txt'.format(i)))
        make_plot(dfs, labels, methods, title='Scattering Network Features Visualisation', n_plots=6)
    plt.savefig('vis_scat/results.png')

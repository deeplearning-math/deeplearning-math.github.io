...
class infoGAN(object):
    def __init__(self, args, SUPERVISED=True):
        ...
        self.test_labels = self.data_Y[8 * self.batch_size: 9 * self.batch_size]
        ...
        if self.gpu_mode:
            self.sample_z_, self.sample_y_, self.sample_c_, self.sample_z2_, \
            self.sample_y2_, self.sample_c2_, self.test_labels = \
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter in range(len(self.data_X) // self.batch_size):
                ...
                if np.mod((iter + 1), 300) == 0:
                    samples = self.G(self.sample_z_, self.sample_c_, self.test_labels)
                    if self.gpu_mode:
        ...
        utils.generate_animation(self.result_dir + '/' + self.model_dir + '/' + self.model_name + '_cont', self.epoch)
    ...
    def visualize_results(self, epoch):
        ...
        utils.save_images(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                          utils.check_folder(self.result_dir + '/' + self.model_dir) + '/' +
                          self.model_name + '_cont_epoch%03d' % epoch + '_test_all_classes.png')
